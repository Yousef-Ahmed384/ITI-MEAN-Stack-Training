...
import { UseCom } from '../models/users';
...
export class Cards {
  ...
  get filteredUsers(): UseCom[] {
    const search = this.searchText.toLowerCase();
    return this.users
      .filter(user => user.email.toLowerCase().includes(search))
      .sort((a, b) => {
        // show matches first
        const aMatch = a.email.toLowerCase().includes(search) ? 0 : 1;
        const bMatch = b.email.toLowerCase().includes(search) ? 0 : 1;
